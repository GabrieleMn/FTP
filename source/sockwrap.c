...
/**********************************************************************************************************/
ssize_t read_f (int fd,void **vptr)
{
	size_t nleft=5;
	ssize_t nread;
	//int dim_i;
	...
	printf("(DEBUG)-start message: %s\n",(char*)ptr);
	
	//leggo dimensione file che sta su 4 byte
	readn(fd,dim,4);
	//dim_i=atoi(ptr);
	printf("(DEBUG)-File dimension: %s\n",(char*)dim);
	

	//*vptr=(char*)malloc(sizeof(char)*dim_i);
	//readn(fd,vptr,dim_i);
...
ssize_t readline_unbuffered (int fd, void *vptr, size_t maxlen)
{
	...
	ptr = vptr;
	for (n=1; n<=maxlen; n++) //ERRORE SU READLINE !!! PRIMA ERA < non <=
	{
